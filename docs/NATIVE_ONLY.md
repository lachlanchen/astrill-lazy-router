...
Its 60-second monitor refreshes status, allowlisted native settings, and
companion-presence markers in one read-only SSH session. The SSH, Astrill
installer, and companion onboarding actions remain available because each has
